...
from .models import ShareAccount, ShareProduct


def share_products_for_user(user):
    queryset = ShareProduct.objects.select_related("institution").order_by(
        "institution__name",
        "code",
        "name",
    )
    ...
    if user.branch_id and user.branch and user.branch.institution_id:
        return queryset.filter(institution_id=user.branch.institution_id)

    return queryset.none()


def share_accounts_for_user(user):
    queryset = (
        ShareAccount.objects.select_related(
            "client",
            "client__branch",
            "client__institution",
            "product",
